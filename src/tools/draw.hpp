...
// bool triangle(Vec3* vecs, Vec2* uvs, Vec3* normals, double buffer[], TGAImage& image,  Model& model) {
...
//                 Vec3 normal = normals[0] * vec.x() + normals[1] * vec.y() + normals[2] * vec.z();
//                 double intensity = normal * light_dir;
//                 if(intensity < 0) continue;
//                 // TGAColor color = model.diffuse().get(uv.x() * model.diffuse().width(), uv.y() * model.diffuse().height());
//                 TGAColor color = {(uint8_t)(255 * intensity), (uint8_t)(255 * intensity), (uint8_t)(255 * intensity)};
//                 image.set(i, j, color);     
//             }
...
// inline Vec3 world2screen(Vec3 vec) {
...
//     Mat4 model;
//     Mat4 view;
//     model << localvhomo(x) << localvhomo(y) << localvhomo(z);
//     view << 1, 0, 0, eye[0],
//              0, 1, 0, eye[1],
//             0, 0, 1, eye[2],
//             0, 0, 0, 1;
